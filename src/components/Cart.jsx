...
import { cart  }from "./config/staticData";

const Cart = () => {
  const [cartData,setCartData] = useState(cart)

    const handleRemove =(id)=>{
      setCartData((prev)=> prev.filter((obj)=> obj.item.id !== id ))
    }
  return (
    <>
    <div className="bg-black text-white p-6 min-h-[85vh]">
      ...
      {cartData.map((product)=>{
        ...
        return(
        <div key={product.item.id} className="flex justify-between bg-[#2D2D2D] mb-4">
